...
import { data_grid_Homescreen_FWalletServices, data_mainFeatures_Homescreen } from "@/src/data/screens/data_home";
      ...
      <FFSafeAreaView>
        ...
        >
          ...
          <View className="items-center gap-2">
            ...
            <FFText style={{ color: "#fff", fontSize: 28 }}>$912.25</FFText>
          </View>
          <FFView className="bg-[#1e1e1e] -mb-10 flex-1 mt-10 rounded-tr-3xl rounded-tl-3xl -ml-4 -mr-4 ">
            <View style={{backgroundColor: theme === 'light' ? '#fff' : '#000' }} className=" shadow-lg shadow-violet-50 w-10/12 -mt-10 rounded-lg flex-row items-center justify-between gap-2 mx-auto  p-2">
         {data_mainFeatures_Homescreen.map((item, index) => (
        <TouchableOpacity 
          key={index}
          onPress={() => item.onPress(navigation)} // Pass the navigation object to onPress
          className="w-16 items-center aspect-square"
        >
          {item.icon}
          <FFText style={{ fontSize: 14, color: "#2E890E" }}>
            {item.title}
          </FFText>
        </TouchableOpacity>
      ))}
            ...
            <View className="mt-6 p-4 gap-4">
              ...
              <View className="flex-row flex-wrap w-full  justify-between gap-4 space-y-10">
                {data_grid_Homescreen_FWalletServices.map((item) => (
                  <TouchableOpacity key={item.title} className="items-center w-[20%]">
